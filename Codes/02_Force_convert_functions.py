...
def GRF_to_OpenSim(
    ...
    for i, file in enumerate(files):
        ...
        for side_file in file:
            ...
            data[side] = filter_data(data[side])
...
def get_IO_dir(subject: str, trials: List[str]) -> Tuple[str, str, Tuple[List[str]]]:
    ...
    input_path = f"../Data/S{subject}/Dynamics/"
    output_path = f"../Outputs/S{subject}/Dynamics/Force_Data/"
    files = [
...
def filter_data(data: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in [" Fx", " Fy", " Fz", " Mx", " My", " Mz"]:
        data.loc[col] = lowpass_filter(data.loc[col])
...
def lowpass_filter(window: Union[pd.DataFrame, np.ndarray], order: int = 4, 
                   lowband: int = 6) -> np.ndarray:
    fs = 100  # Hz
    low_pass = lowband/(fs*0.5)
    b, a = signal.butter(N=order, Wn=low_pass, btype="lowpass")
